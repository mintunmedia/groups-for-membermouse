	...
	<table class="widefat" id="mm-data-grid" style="width:800px;">
		...
		<tbody>
			<?php foreach ($results as $res) :
				...
				$activeSql				= "SELECT count(id) AS active FROM " . $wpdb->prefix . "group_sets_members WHERE group_id = '" . $res->id . "' AND member_status = 1";
				$activeResult			= $wpdb->get_row($activeSql);
			?>
				<tr>
					<td><a href="admin.php?page=groupsformm&type=manage_group&group_leader=<?= $group_leader_id; ?>"><?php if (!empty($res->group_name)) : echo $res->group_name;
																																																						else : echo "Group";
																																																						endif; ?></a></td>
