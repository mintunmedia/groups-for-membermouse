jQuery(function ($) {
  ...
  $(document).on(
    "click",
    ".confirmationpage_notice .notice-dismiss",
    function () {
      jQuery.ajax({
        url: dismiss_notices.ajax_url,
        data: {
  ...
  jQuery("a#create_group").click(function () {
    ...
    jQuery
      ...
      .done(function (data) {
        ...
        jQuery("#create_group_content").css({
          top: contentTop,
          left: contentLeft,
        });
  ...
  MGROUP = {
    ...
    saveGroupForm: function (groupId) {
      ...
      if (leader_memlevel == "") {
        ...
        $("#leadermemlevelErr").html(
          "Please select the Group Leader Associated Access (Membership Level)."
        );
        retVar = true;
      } else {
      ...
      if (lCost == 1 && leader_cost == "") {
        ...
        $("#groupLeaderCostErr").html(
          "Please select the Group Leader Associated Cost (Product)."
        );
        retVar = true;
      } else {
      ...
      if (member_memlevel == "") {
        ...
        $("#membermemlevelErr").html(
          "Please select the Group Member Associated Access (Membership Level)."
        );
        retVar = true;
      } else {
      ...
      if (mCost == 1 && member_cost == "") {
        ...
        $("#groupMemberCostErr").html(
          "Please select the Group Member Associated Cost."
        );
        retVar = true;
      } else {
          ...
          success: function (data) {
            $.each(data, function (i) {
              if (i == "name") {
                ...
                if (data[i] == "yes") {
                  $(".group-loading-container").hide();
                  $("#group_popup_msg").html(
                    '<div class="group_success">Success! Group Saved.</div>'
                  );
                  ...
                  $(".group-loading-container").hide();
                  $("#group_popup_msg").html(
                    '<div class="group_failure">Uh oh! An error occured. Please try again.</div>'
                  );
                  $("#group_popup_msg").show();
                }
              }
          ...
          success: function (data) {
            $.each(data, function (i) {
              if (data[i] == "yes") {
                ...
                $("#create_group_background").hide();
                window.location =
                  "admin.php?page=groupsformm&type=manage&msg=1";
              } else if (data[i] == "no") {
                $("#create_group_loading").hide();
                $("#create_group_background").hide();
                window.location =
                  "admin.php?page=groupsformm&type=manage&msg=2";
              }
            });
          },
        });
            ...
            $.each(data, function (i) {
              if (i == "group_size") {
                ...
                if (data[i] == "yes") {
                  $(".group-loading-container").hide();
                  $("#group_popup_msg").html(
                    '<div class="group_success">Update successful.</div>'
                  );
                  $("#group_popup_msg").show();
        ...
        success: function (data) {
          ...
          $.each(data, function (i) {
            if (i == "error") {
              ...
              $("#add_user_msg").show();
              $("#add_user_container").html(
                '<a class="group-button" title="Check Availability" onclick="javascript:MGROUP.checkUsername(\'' +
                  group_id +
              ...
              $("#add_user_msg").show();
              $("#add_user_container").html(
                '<a class="group-button button-green" title="Add Member" id="add_user_button" onclick="javascript:MGROUP.addGroupUsers(\'' +
                  group_id +
                  "','" +
                  i +
                  "');\">Add Member</a>"
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $.each(data, function (i) {
            if (data[i] == "yes") {
              $("#add_user_msg").html(
                '<font class="green-text">Success! We added the member.</font>'
              );
              $("#add_user_msg").show();
              $("#username").val("");
              $("#user_id").val("0");
              ...
              $("#add_user_msg").html(
                '<font class="red-text">An error occured. Please try again later.</font>'
              );
              $("#add_user_msg").show();
              $("#username").val("");
              $("#user_id").val("0");
            ...
            $.each(data, function (i) {
              if (i == "group") {
                ...
                if (data[i] == "yes") {
                  $(".group-loading-container").hide();
                  $("#group_popup_msg").html(
                    '<div class="group_success">Success! We created the group.</div>'
                  );
                  $("#group_popup_msg").show();
    ...
    changeGroupMemberCost: function (levelId) {
      ...
      $.ajax({
        ...
        data:
          "levelId=" + levelId + "&type=member&_wpnonce=" + rest_nonce._wpnonce,
        success: function (data) {
    ...
    changeGroupLeaderCost: function (levelId) {
      ...
      $.ajax({
        ...
        url: change_group_cost.ajax_url,
        data:
          "levelId=" + levelId + "&type=leader&_wpnonce=" + rest_nonce._wpnonce,
        success: function (data) {
