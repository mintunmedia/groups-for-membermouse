...
define('MGROUPS_PREFIX', $useLegacyDB == 1 ? '' : $wpdb->prefix);
...
class MemberMouseGroupAddon {
	...
	function __construct() {
		...
		if ($this->is_plugin_active(self::MM_PLUGIN_PATH)) {
			...
			add_action('admin_notices', [$this, 'groupsformm_notice_mmrequired']);
		}
	}
	...
	public function MemberMouseGroupMemberAdded($data) {
		...
		if (isset($data["cf_" . $groupId]) && !empty($data["cf_" . $groupId])) {
			...
			if (is_numeric($cf)) {
				...
				// Check if Leader already has a group and only create if it doesn't exist yet.
